...
void CodeGenerator::Generate() {
    _asmCodeString = _template + Traversal(_tree.GetRoot());
    std::cout << _asmCodeString << std::endl;
}
...
std::pair<bool, unsigned int> CodeGenerator::HasInUpper(const std::string &id) {
    ...
    do {
        if (tmp->idTable.Has(id))
            return std::make_pair(true, typeid(MasmID_Data).hash_code());
        if (tmp->arrayTable.Has(id))
            return std::make_pair(true, typeid(MasmArray_Data).hash_code());
        tmp = tmp->upperBlock;
    } while (tmp);
...
std::pair<bool, std::string> CodeGenerator::GetUID(ProgramBlock<MasmID_Data, MasmArray_Data>* const &block, const std::string &id) {
    std::pair<bool, std::string> element;
    for (auto &internalBlock : block->internalBlocks) {
        element = GetUID(&internalBlock, id);
        if (element.first)
            return element;
    }

    auto idTable = block->idTable.GetTable();
    auto arrTable = block->arrayTable.GetTable();
    for (auto idData = idTable.rbegin(); idData != idTable.rend(); idData++) {
        if (idData->id == id)
            return std::make_pair(true, idData->uid);
    }
    for (auto arrData = arrTable.rbegin(); arrData != arrTable.rend(); arrData++) {
        if (arrData->id == id)
            return std::make_pair(true, arrData->uid);
    }
...
std::string CodeGenerator::VariableDeclaration(Node *const &node) {
    ...
    auto tmpBlock = _currentBlock;
    while (tmpBlock->upperBlock)
        tmpBlock = tmpBlock->upperBlock;
    auto lastUid = GetUID(tmpBlock, id);
    if (lastUid.first)
        uid = lastUid.second + "_";
    if (HasInReservedWordsTable(id))
        uid += "_";
...
std::string CodeGenerator::Print(Node *const &node) {
    ...
    for (const auto& expr : exprList) {
        if (expr->GetData()->ruleType == RuleType::Identifier) {
            id = expr->GetData()->token.GetValue();
            varData = HasInUpper(id);
            if (varData.first && varData.second == typeid(MasmID_Data).hash_code()) {
                ...
                if (idData.type.first == MASMType::DWORD)
                    parameters.push("\tpush " + idData.id + idData.uid + "\n");
            ...
            else if (varData.first && varData.second == typeid(MasmArray_Data).hash_code()) {
...
std::string CodeGenerator::PrintArray(const MasmArray_Data &arrayData) {
    ...
    code += "\t\t.if (ebx != " + std::to_string(arrayData.elementCount-1) + ")\n";
    code += "\t\t\tprintf(\", \")\n";
    code += "\t\t.endif\n";
    code += "\t\tinc ebx\n";
    code += "\t.endw\n";
...
void CodeGenerator::CreateAsmFile() {
    ...
    system("cd TempCompile && makeit.bat");
}